...
public class SRLCostAndGradient extends AbstractCachingDiffFunction {
	...
	public SRLCostAndGradient(RNTNModel model, List<Tree> trainingBatch) {
		...
		if(trainingBatch != null){
			...
			this.sentPostags = Generics.newArrayList();
			for (Tree tree : trainingBatch){
				...
				this.sentPostags.add(RNNCoreAnnotations.getPOSTags(tree));
			}
		}
	}
	...
	public void calculate(double[] theta) {
		...
		for (int i=0; i<trainingBatch.size(); i++){	
			...
			for(int nverbs=0; nverbs < verbIndices.get(i).size(); nverbs++){// verbIndex : verbIndices.get(i)){
				...
				setPosFeatures(leaves, sentPostags.get(i));
	...
	public void setPosFeatures(List<Tree> leaves, POSTags posTags) {
		
		for(int i=0 ; i< leaves.size(); i++){
			Tree tree = leaves.get(i);
			String wordStr = tree.label().value();
			SimpleMatrix nodeVector = model.getWordVector(wordStr);
			Integer posId = posTags.get(i);
			String binary = posId.toBinaryString(posId);
			
			int row = nodeVector.numRows() - 3;  //last two dimensions are for verbindex and wordindex features
			for(int j=0; j<6; j++) { //total pos tags = 46 so using 6 dimensions
				nodeVector.set(row-j, 0, 0);
			}
			
			for(int j=0; j<binary.length(); j++) { //total pos tags = 46 so using 6 dimensions
				nodeVector.set(row-j, 0, Double.parseDouble(String.valueOf(binary.charAt(binary.length()-j-1))));
			}
	...
	public List<Integer> getInnerOuterContext(Tree tree, Tree subtree, int windowSize, int vid, int wid){
	...
	public List<Integer> getCatInputNodes(Tree tree, Tree subtree, int windowSize, int vid, int wid) {
		...
		for(int i=1 ; i <= windowSize; i++){
			if(vid-i < 0)
				...
				label = (CoreLabel)leaves.get(vid-i).label();
				nodes.add(label.get(IndexAnnotation.class));
				//						nodes.add(leaves.get(wid-i));
			}
		}		
	...
	private void backpropDerivativesAndError(Tree tree, Tree subtree, 
		...
		softmaxCD = topDelta.mult(RNNUtils.concatenateWithBias(hiddenValues).transpose());
		...
		//		SimpleMatrix catInputDerivative = RNNUtils.elementwiseApplyTanhDerivative(catInput);
		SimpleMatrix deltaFromClass = wHidden.transpose().mult(deltaH);
		//		deltaFromClass = deltaFromClass.extractMatrix(0, deltaFromClass.numRows(), 0, 1).elementMult(catInputDerivative);
		for (Tree node : tree){
	...
	public void forwardPropagateTree(Tree tree) {
		...
		if (tree.isLeaf()) {
			...
			//			nodeVector = RNNUtils.elementwiseApplyTanh(nodeVector); // non-linearity 
