...
/**
 * Recursive Neural Tensor Model, Socher, EMNLP 2013 
 * 
 * @author bhanu
 *
 */
public class RNTNModel implements Serializable {
	/**
	...
	/**
	 * Cx(3xN+1), where N = size of word vectors, C is the number of classes
	 * Input is vectors of topnode, verbnode and wordnode plus bias
	 */
	public Map<String, SimpleMatrix> unaryClassification;

	/**
	 * Extra hidden layer to increase number of parameters for classifying roles
	 */
	...
	public final int fanIn; //bhanu

	public final int nHidden;
	...
	static final String PADDING = "*PAD*";
	 ...
	 */
	static RNTNModel modelFromMatrices(SimpleMatrix W, SimpleMatrix Wcat, SimpleTensor Wt, SimpleMatrix Wh, Map<String, SimpleMatrix> wordVectors, RNNOptions op) {
		...
		TwoDimensionalMap<String, String, SimpleMatrix> wHidden = TwoDimensionalMap.treeMap();
		unaryClassification.put("", Wh);

		return new RNTNModel(binaryTransform, binaryTensors, binaryClassification, unaryClassification, wHidden, wordVectors, op);
	}

	private RNTNModel(TwoDimensionalMap<String, String, SimpleMatrix> binaryTransform,
		...
		unaryClassificationSize = numClasses * (nHidden+1);
		wHiddenSize = nHidden * fanIn;
	 ...
	 */
	public RNTNModel(RNNOptions op, List<Tree> trainingTrees) {
		this.op = op;
		this.fanIn = op.fanIn;
		this.nHidden = op.nHidden;
		this.rae = new RAEModel();
		...
		for (Pair<String, String> binary : binaryProductions) {
			...
			wHidden.put(left, right, randomHiddenMatrix());
	...
	SimpleMatrix randomTransformBlock() {
		...
		}else{// prior is the average of two words
			double range = 0.001;
	...
	SimpleMatrix randomClassificationMatrix() {
		SimpleMatrix score = new SimpleMatrix(numClasses, nHidden+1); 
		// Leave the bias column with 0 values
		double range = 1.0 / (Math.sqrt((double) numHid));
		score.insertIntoThis(0, 0, SimpleMatrix.random(numClasses, nHidden, -range, range, rand));
		return score.scale(op.trainOptions.scalingForInit);
	}

	SimpleMatrix randomHiddenMatrix(){
		SimpleMatrix score = new SimpleMatrix(nHidden, fanIn);

		double range = 1.0 / (Math.sqrt((double) numHid));
		score.insertIntoThis(0, 0, SimpleMatrix.random(nHidden, fanIn-1, -range, range, rand));
		return score.scale(op.trainOptions.scalingForInit);
	}
	...
	public double[] paramsToVector() {
		...
		return RNNUtils.paramsToVector(totalSize, binaryTransform.valueIterator(), binaryClassification.valueIterator(), 
				SimpleTensor.iteratorSimpleMatrix(binaryTensors.valueIterator()), unaryClassification.values().iterator(), wHidden.valueIterator(),
				wordVectors.values().iterator());
	}

	public void vectorToParams(double[] theta) {
		RNNUtils.vectorToParams(theta, binaryTransform.valueIterator(), binaryClassification.valueIterator(), 
				SimpleTensor.iteratorSimpleMatrix(binaryTensors.valueIterator()), unaryClassification.values().iterator(), wHidden.valueIterator(),
				wordVectors.values().iterator());
	}
	...
	public static RNTNModel loadSerialized(String path) {
		try {
