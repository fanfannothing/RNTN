...
public class PhraseRepTraining {
	...
	public static double executeOneTrainingBatch(RNNPhraseModel model, List<Tree> trainingBatch, double[] sumGradSquare) {
		ReconstructionCostAndGradient gcFunc = new ReconstructionCostAndGradient(model, trainingBatch);
...
//		double currCost = gcFunc.valueAt(theta);
	...
	public static void train(RNNPhraseModel model, String modelPath, List<Tree> trainingTrees, List<Tree> devTrees) {
...
//					System.out.println(score);
					...
					//compare models evalutiona
//					String trainedRAEModelName = "data/models/srl/rae_uf_cost333.mo-0028";					
//					RNNPhraseModel raeModel = RNNPhraseModel.loadSerialized(trainedRAEModelName);
//					CompareModels cm = new CompareModels();
//					double score = cm.evaluate(raeModel, model, trainingTrees.subList(0, 10));
//					System.out.println(score);
...
//		List<Tree> devTrees = SRLUtils.readTrees(devPath);
...
//			model.encodeTransform = model2.encodeTransform;
		...
		if (runTraining) {
			train(model, modelPath, trainingTrees, null);
			model.saveSerialized(modelPath);
		}
	}
