...
import edu.stanford.nlp.rnn.RNNCoreAnnotations.POSTagsMap;
...
public class SRLUtils {
	...
	public static void attachTagsNVerbIndexes(Tree tree, List<String> verbIds,
			ArrayList<List<String>> tagslists, Categories categories, List<String> pos, POSTagsMap posmap){
		...
		for(String postag : pos){
			if(! postag.equalsIgnoreCase(""))
				postags.add(posmap.get(postag));
			//			else
			//				sentVerbs.add(null);
		}
		...
		RNNCoreAnnotations.setPOSTags(tree, postags);
	 ...
	 * Given a file name, reads in those trees and returns them as a List
	 */
	public static List<Tree> readTreesWithGoldLabels(String treespath, String verbIDsPath, String labelsPath, String posPath) {
		...
		POSTagsMap posmap = POSTagsMap.getPOSMap();

		//read pos for each sentence
		List<List<String>> pos = Generics.newArrayList();
		for (String line : IOUtils.readLines(posPath, "utf-8")) {
			String[] postags = line.split("\\|");
			pos.add( Arrays.asList(postags));
		}
		...
		for (int i =0; i < treebank.size(); i++) {
			...
			attachTagsNVerbIndexes(tree, verbIndexes.get(i), allVerbTags.get(i), categories, pos.get(i), posmap);
