...
public class RNNCoreAnnotations {
	...
	public static class POSTags extends ArrayList<Integer>{}
	public static class POSTagsMap extends HashMap<String,Integer>{

		public static POSTagsMap getPOSMap() {
			
			POSTagsMap posmap = new POSTagsMap();
			try{
				FileReader reader = new FileReader(new File(Config.POS_TAGS));
				BufferedReader bfr = new BufferedReader(reader);
				String key = bfr.readLine();
				int posid = 0;
				while(key!=null){					 
					key = key.trim();
					posmap.put(key, posid);
					key = bfr.readLine();
					posid+=1;
				}
	...
	public static class POSAnnotation implements CoreAnnotation<POSTags>{
		public Class<POSTags> getType(){
			return POSTags.class;
		}
	}
	...
	public static POSTags getPOSTags(Tree tree) {
		Label label = tree.label();
		if (!(label instanceof CoreLabel)) {
			throw new IllegalArgumentException("CoreLabels required to get the attached Verb Indexes");
		}
		return ((CoreLabel) label).get(POSAnnotation.class);
	}
	public static void setPOSTags(Tree tree, POSTags postags){
		Label label = tree.label();
		if (!(label instanceof CoreLabel)) {
			throw new IllegalArgumentException("CoreLabels required to set the attached Verb Indexes");
		}
		((CoreLabel) label).set(POSAnnotation.class, postags);
	}
