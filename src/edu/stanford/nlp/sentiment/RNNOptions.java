...
public class RNNOptions implements Serializable {
	 ...
	 */
	public int randomSeed = 42;//(new Random()).nextInt();
	 ...
	 */
	public String unkWord = "UNKNOWN";
	public String padWord = "PADDING";
	...
	public int numHidRAE = 100;

	public final int nWordsInContext = 2;
	...
	public int numClasses = 35;  //roleCats.size(); // 

	public boolean lowercaseWordVectors = false;

	public boolean useTensors = true;

	public int windowSize = 2; //bhanu

	public int fanIn = (windowSize*2*2 + 1 + 1 + 1)* numHid + 1;  //3*numHid  + 1; //  // windowSize words left and right of the word-to-tag and verb, word-to-tag, topNode and verbNode and bias
	public int nHidden = 100;  // size of the hidden layer, to be added at the top node, for classification
	...
	public String toString() {
		...
		result.append("nHidden=" + nHidden + "\n");
		result.append("windowSize=" + windowSize + "\n");
	...
	public int setOption(String[] args, int argIndex) {
		if (args[argIndex].equalsIgnoreCase("-randomSeed")) {
			...
			return argIndex + 2;
		} else if (args[argIndex].equalsIgnoreCase("-windowSize")) {
			windowSize = Integer.valueOf(args[argIndex + 1]);
			return argIndex + 2;
		} else if (args[argIndex].equalsIgnoreCase("-nHidden")) {
			nHidden = Integer.valueOf(args[argIndex + 1]);
			return argIndex + 2;
